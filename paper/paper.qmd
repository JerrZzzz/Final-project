...
residuals_data <- data.frame(
  Observation = 1:length(residuals(model2_11th)), # Creating an index for each observation
  Residuals = residuals(model2_11th) # Extracting residuals
)
...
model1_qqplot <- ggplot(model1_residuals, aes(sample = Residuals)) +
  geom_qq() +
  geom_qq_line(color = "red") +
  labs(title = "QQ Plot of Residuals", x = "Theoretical Quantiles", y = "Sample Quantiles") +
  theme_minimal()

slp <- ggplot(data.frame(Fitted=model2_11th$fitted.values, SqrtAbsResid=sqrt(abs(residuals(model2_11th)))), aes(x=Fitted, y=SqrtAbsResid)) +
  geom_point() +
